...
Este README es la **referencia principal del proyecto**: recoge qué es Society, las decisiones vigentes, la estructura prevista y las herramientas. Los documentos de la [sección 14](#14-documentos-del-proyecto) contienen el análisis y la evidencia que respaldan cada decisión.
...
9. [Estrategia de redes y medición](#9-estrategia-de-redes-y-medición)
10. [Cumplimiento normativo](#10-cumplimiento-normativo)
11. [Calendario orientativo](#11-calendario-orientativo-hasta-enero-de-2027)
12. [Riesgos principales](#12-riesgos-principales)
13. [Decisiones](#13-decisiones)
14. [Documentos del proyecto](#14-documentos-del-proyecto)
15. [Estado actual y próximos pasos](#15-estado-actual-y-próximos-pasos)
...
- **Torre de Vega no es el objetivo del proyecto.** Es el caso práctico real en el que se probó el flujo a mano y del que salen evidencias y aprendizajes. Sus reglas (no generar fuego, acabado editorial dramático, preferencias de manos…) son **preferencias de ese cliente**. En Society se guardan como reglas por restaurante, no como reglas del producto.
...
- **Audiencia.** El cliente de Society es el **hostelero**, no el comensal. La métrica principal de esta cuenta serían solicitudes de demo o altas, no reservas.
- **Redes.** Instagram primero; valorar TikTok, Facebook o LinkedIn.
- **Línea editorial y material de partida.** El [informe de estrategia de redes §4.8](informes/Society_estrategia_redes_y_retroalimentacion.md) propone pilares iniciales.
...
- Declarar el contenido generado con IA cuando corresponda (ver [sección 10](#10-cumplimiento-normativo)).
...
**Cuánto cuesta producir cada plan.** Con Seedance 2.5 como modelo principal, el vídeo es la partida que manda:

| Unidad | fal 1080p | fal 720p | Higgsfield 1080p (pendiente de contrato) |
|---|---|---|---|
| Reel estándar (seis planos generados) | 44,75 USD | 19,88 USD | 19,05 USD |
| Clip de historia de 5 s | 9,04 USD | 3,86 USD | 3,68 USD |

Una imagen final independiente cuesta unos **0,31 USD**.

Con cuotas de ejemplo (plan 2 con 20 imágenes y 4 clips de historia; plan 3 con 4 reels), el coste por restaurante sería:
...
- Precio mensual y si el alta (onboarding) se cobra aparte.
...
- **Los planes 1 y 2 usan Google Business Profile y directorios**, que no son el foco de desarrollo hasta enero. Al lanzar, esas partes se gestionarían de forma asistida hasta automatizarlas.
...
1. **Onboarding.** El restaurante configura una vez su identidad, platos, ubicaciones, estilo y restricciones. Se captan fotos y clips reales (plato en tres cuartos, cenital y detalle; lugar donde se sirve; acciones difíciles como servir o cortar). Cada recurso queda etiquetado como real, generado o externo.
2. **Tres entradas de creación**: *inspirarme en un reel* (se analiza la estructura y se adapta con recursos propios), *crear desde un plato* y *comunicar una novedad* (fecha, oferta o mensaje confirmado).
...
5. **Producción.** Contrato de plano y elección de técnica antes que de modelo: material real, imagen a vídeo con Seedance 2.5, estados inicial y final con Seedance 2.5, composición en After Effects o 3D experimental. Voz en off y subtítulos cuando la pieza los necesita.
...
7. **Publicación.** Asistida, con lista de verificación. La publicación automática por API se valora más adelante.
...
| Archivos | Cloudflare R2 | Fotos, clips, pistas de voz, entregables |
...
| Tareas programadas | Supabase Cron + Edge Functions | Capturas de métricas, renovación de tokens |
| Worker de montaje | Windows con After Effects (render automatizado) | Montaje en plantillas, rótulos y subtítulos |
| Worker de medios | Linux | Análisis de vídeo, inspección con FFmpeg, transcripción y alineación de subtítulos, 3D experimental |
...
- **Trabajos recuperables e idempotentes.** Una caída no repite generaciones pagadas.
- **Reglas de marca como datos versionados**, con los artefactos que dependen de cada regla.
- **Claves de proveedores solo en el servidor.**
- **Presupuesto reservado por proyecto y por plan.**

### 7.1 Modelo de datos: entidades principales
...
| Presencia local (planes 1 y 2) | Ficha de Google, Horario especial, Reseña, Respuesta a reseña, Ficha externa (TripAdvisor y otras) |
| Resultados | Evento de conversión (canal, código de campaña, pieza atribuida), sin datos personales innecesarios |

Horarios, carta, fotos reales y calendario de novedades son **datos únicos** que se publican en varios canales (redes, ficha de Google, otras plataformas). Dejarlo previsto ahora evita rehacer tablas al añadir los planes 1 y 2.

## 8. Herramientas

Estado a 15 de septiembre de 2026. Tarifas detalladas en el [informe de viabilidad §9](informes/Society_diseno_y_viabilidad.md).

| Función | Herramienta | Estado |
|---|---|---|
| Planificación, prompts, revisión y dirección de edición | GPT-6 Astra (API de OpenAI) | Propuesta |
| Generación y edición de imágenes | Nano Banana Pro (Gemini API), hasta 14 imágenes de referencia | Propuesta |
| **Vídeo generado (todas las rutas)** | **Seedance 2.5**: imagen inicial, imagen inicial y final, referencias, clips de 4 a 30 s, audio sincronizado opcional | **Modelo principal** (decisión del 15/09/2026). Resolución (720p o 1080p) y proveedor (fal, Higgsfield u otro) pendientes |
| Alternativas de vídeo documentadas | Seedance 2.0, Kling 3.0 Pro, Veo 3.1 | Solo si un ensayo concreto lo justifica |
...
| Subtítulos | Marcas de tiempo de la voz generada; WhisperX o ElevenLabs Scribe cuando el audio no sale del guion; incrustados en After Effects o FFmpeg | Propuesta |
...
| Instagram | Instagram API con Instagram Login: publicación, estadísticas, comentarios | Acceso estándar para la cuenta propia; revisión de Meta (App Review) para cuentas de clientes |
...
- **Posición en la clasificación.** En Arena (14/09/2026) es cuarto en imagen a vídeo y sexto en texto a vídeo, prácticamente empatado con Seedance 2.0. Está en el grupo de cabeza y aporta clips de hasta 30 s, fotograma final y audio sincronizado.
- **Coste.** A 1080p en fal cuesta 1,164 USD por segundo, frente a 0,473 USD a 720p.
- **Pendiente.** El ensayo de calidad del TFG compara 720p reescalado y 1080p nativo con platos y locales reales.

## 9. Estrategia de redes y medición
...
- **Señales que Instagram reconoce:** tiempo de visionado, me gusta por alcance (más importante con seguidores) y envíos por alcance (más importante con no seguidores).
...
| Voz de personas reales | La Ley Orgánica 1/1982 exige consentimiento expreso para usar la voz de una persona con fines publicitarios; ElevenLabs prohíbe clonar voces sin consentimiento | Consentimiento escrito por voz y finalidad; voces diseñadas o de biblioteca por defecto |
...
| Reseñas | Google prohíbe incentivos y presión; la Ley de Competencia Desleal (arts. 27.7 y 27.8) prohíbe reseñas falsas | Society nunca genera, filtra ni condiciona reseñas |
...
## 11. Calendario orientativo hasta enero de 2027
...
|---|---|---|
| 15 sep – 4 oct | Cerrar la definición | Cuotas de los planes, resolución y proveedor de Seedance 2.5, modelo de datos, elección React o Angular, solicitudes de acceso a Meta y Google, cuenta propia de Society creada |
...
| 30 nov – 20 dic | Vídeo y reels (plan 3) | Clips de Seedance 2.5 (imagen inicial, e inicial y final), voz en off y subtítulos, montaje automático en plantillas, medición en Instagram |
...
| **Coste del vídeo con Seedance 2.5** (plan 3): a 1080p en fal, un reel cuesta unos 44,75 USD | Decidir resolución y proveedor con el ensayo; resolver planos con material real; límite de reels y de planos generados por plan; reintentos acotados |
...
| 2026-09-15 | Seedance 2.5 es el modelo principal para generar vídeo | Sección 8 y [viabilidad §4.2](informes/Society_diseno_y_viabilidad.md) |
...
2. **Resolución (720p o 1080p) y proveedor de Seedance 2.5**, que deciden el coste del plan 3.
...
8. Redes para clientes además de Instagram (Facebook, TikTok).
...
|---|---|---|
| [`idea.md`](./idea.md) | Definición inicial: problema, propuesta de valor, flujo por fases | Parcialmente superado: el 3D pasa a experimental y el predictor de viralidad no se usa como probabilidad de éxito |
| [`caso-practico-01-torre-de-vega.md`](./caso-practico-01-torre-de-vega.md) | Primera ejecución real del flujo completo y qué implica para Society | Registro histórico con actualización del 14/09 |
| [`investigacion-calidad-profesional-reels-ia.md`](./investigacion-calidad-profesional-reels-ia.md) | Cómo acercar el vídeo con IA a calidad de agencia: 3D, transferencia de movimiento, relighting, posproducción, verificación | Investigación de referencia |
| [`base-conocimiento-torre-de-vega/`](./base-conocimiento-torre-de-vega/) | Reglas, metodología de prompting, automatización de After Effects, guiones e informes reales del caso práctico | Evidencia; no define el producto |
| [`informes/Society_diseno_y_viabilidad.md`](informes/Society_diseno_y_viabilidad.md) | Diseño del producto, método de vídeo, arquitectura, costes y viabilidad | Actualizado el 15/09 con Seedance 2.5 y coste por plan; el PDF corresponde a la versión del 14/09 |
| [`informes/Society_inventario_documental.md`](informes/Society_inventario_documental.md) | Inventario de los 65 archivos analizados | Referencia |
| [`informes/Society_estrategia_redes_y_retroalimentacion.md`](informes/Society_estrategia_redes_y_retroalimentacion.md) | Estrategia de redes, métricas, ciclo de retroalimentación, cuenta propia, herramientas por fases, cumplimiento | Actualizado el 15/09 |
| [`informes/Society_auditoria_estrategica_y_mejoras.md`](informes/Society_auditoria_estrategica_y_mejoras.md) | Campos estratégicos de una agencia de hostelería, huecos, vulnerabilidades, formación | Actualizado el 15/09 con los planes decididos |
| [`informes/Society_voces_y_subtitulos.md`](informes/Society_voces_y_subtitulos.md) | ElevenLabs, servicios en la nube y modelos gratuitos de voz; subtítulos; comparativa, costes, cumplimiento y prueba a ciegas | Nuevo (15/09) |

## 15. Estado actual y próximos pasos

📌 **Fase de definición** (15 de septiembre de 2026). No hay código ni stack cerrado.

**Próximos pasos:**

1. Cerrar cuotas y precios de los tres planes con los costes del informe de viabilidad.
2. Decidir resolución y proveedor de Seedance 2.5; confirmar en la primera prueba las dimensiones reales y el importe facturado.
...
6. Cerrar el modelo de datos con las entidades de la sección 7.1.

La base de conocimiento está sincronizada con el caso práctico Torre de Vega a fecha de 2026-09-14 (ver la tabla de sincronizaciones en [`base-conocimiento-torre-de-vega/README.md`](./base-conocimiento-torre-de-vega/README.md)).
